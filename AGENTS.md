...
### Implemented poly struct type inference from field values (Phase 14C)
- **Feature**: `b := Box{val=42}` now infers `T=int` without explicit type arguments. Works with multiple type params (`Pair{first=10, second=20.0}` → `Pair(int, f64)`, `Triple{a=1, b=2.0, c=3}` → `Triple(int, f64, int)`).
- **Implementation** (`sem_evaluate_expr.c`): Added `sem_infer_poly_struct_type()` — when `sem_resolve_type_expr` fails for a bare `Identifier` type prefix in a struct literal, checks if the identifier is a polymorphic struct symbol. If so: (1) collects struct field definitions from the origin StructType AST, (2) evaluates all literal field value expressions to get their types, (3) matches struct definition fields whose type is a bare poly param name (e.g., `val: T`) against literal field values, (4) builds a `PolyEnv` binding each `$T` to the matching value's type, (5) pushes env + calls `sem_resolve_struct_type` + pops env.
- **API changes**: Made `sem_resolve_struct_type` and `collect_parameters_from_param_list` non-static, declared in `sem_type_resolver.h` for cross-file access.
- **Limitations**: Only infers type params (`$T: typeid`) from fields whose type IS the bare poly param name (e.g., `val: T`). Cannot infer int params (`$N: int`) from field values (would need `len(arr)` evaluation). Cannot infer from complex field types like `[$N]T` or `[]T` (would need structural unification of field type pattern against value type).
- **Tests**: `test_poly_struct_inference.odin` (8 subtests: single param int/f64, two params mixed types, three params, field assignment, passing inferred struct to function, explicit+inferred coexistence, same-type pair). **All 186 tests pass** (185 previous + 1 new).

### Poly struct completeness: dedup cache, cross-function usage, bug fixes
- **Poly struct TypeDescriptor dedup cache** (`sem_type_resolver.c`): Added `PolyStructCacheEntry` static cache (256 entries) in `sem_resolve_type_application`. Keyed by (origin AST pointer, type arg TypeDescriptor pointers, int arg values). Before full resolution, checks cache for identical previous instantiation and returns the cached TypeDescriptor. After resolution, stores result. This ensures `Box(int)` in a function signature and `Box(int)` at the call site share the same TypeDescriptor pointer, enabling pointer-equality type comparisons throughout the semantic analyser.
...
- **Tests**: `test_poly_struct_extended.odin` (8 subtests: `^T` field + deref, `len()` on `[$N]T` field, poly struct as function arg, poly struct return from function, `^Box(int)` pointer arg, `PtrBox(f64)` with `^T` deref, `[]T` slice field with `make()`/`len()`, for-range accumulation through `[$N]T` field). **All 185 tests pass** (184 previous + 1 new).
...
- **Tests**: `test_struct_literal.odin` (regular `Vec{x=1,y=2}`, poly `Box(int){val=42}`, assignment form `v = Vec{x=10,y=20}`). **All 184 tests pass** (no regressions).

### Fixed multi-field struct syntax (`x, y: int`)
- **Root cause**: `sem_resolve_struct_type` inner loop in `sem_type_resolver.c` treated the second Identifier in a multi-name StructField as the type node (via `else if (child->type == AST_NODE_IDENTIFIER && name_node != NULL) type_node = child`). For `x, y: int`, this set `type_node = "y"` then overwrote it with `BasicType "int"`, losing the name `y`. Only the first field `x` was registered.
- **Fix**: Rewrote the inner loop to collect all Identifier children into a `name_nodes[]` array. First non-Identifier, non-Directive child is the type. If all children are Identifiers, the last one is the type (handles `val: T` in poly structs). Each name is registered as a separate field with the same resolved type. Also fixed `field_count` pre-calculation to count total Identifier names (not just StructField nodes). Fixed `#align` directive skipping — added `ci++` to skip the IntegerValue child after the Directive.
