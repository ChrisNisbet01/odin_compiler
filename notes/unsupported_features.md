...
## Recently fixed
- **Struct literal construction (`Vec{x=1, y=2}`, `Box(int){val=42}`)**: Moved `StructLitExpr` out of `PrimaryExpression` to fix PEG greedy-parsing regression (parser consumed `{` in if/for/switch conditions as struct literal body). Created transparent `ExpressionOrStructLit = StructLitExpr | Expression` rule used in `VariableDecl`, `AssignStatement`, and `ArgumentList`. Regular and polymorphic struct literals work in init and assignment forms.
- **`bool` in `print_value`**: Added `type_of(v) == type_of(bool)` case to `print_value` in `stubs/core/fmt/fmt.odin`. Previously printed `<?>` for boolean values passed through `..any` variadic arguments.
