...
@(require_results)
matrix4x4_determinant :: proc "contextless" (m: matrix[4, 4]$T) -> (det: T) {
...
@(require_results)
matrix5x5_determinant :: proc "contextless" (m: matrix[5, 5]$T) -> (det: T) {
	return matrix_determinant_generic(m)
}

@(require_results)
matrix6x6_determinant :: proc "contextless" (m: matrix[6, 6]$T) -> (det: T) {
	return matrix_determinant_generic(m)
}

@(require_results)
matrix7x7_determinant :: proc "contextless" (m: matrix[7, 7]$T) -> (det: T) {
	return matrix_determinant_generic(m)
}

@(require_results)
matrix8x8_determinant :: proc "contextless" (m: matrix[8, 8]$T) -> (det: T) {
	return matrix_determinant_generic(m)
}

@(require_results)
matrix_determinant_generic :: proc "contextless" (a: matrix[$N, N]$T) -> T {
	when N == 1 {
